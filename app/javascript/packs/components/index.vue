<template>
    <div>
        <div class="row">
            ...
            <div class="col s2 m1" v-on:click="createTask">
        ...
        <div>
            <ul class="collection">
                <li v-for="task in tasks" v-if="!task.is_done" v-bind:id="'row_task_' + task.id" class="collection-item">
                    ...
                    <label v-bind:id="'task_' + task.id">{{ task.name }}</label>
                </li>
            </ul>
        </div>
        <!-- finished task display button -->
        <div class="btn" v-on:click="displayFinishedTasks">Display finished tasks</div>
        <!-- finished task list -->
        <div id="finished-tasks" class="display_none">
            <ul class="collection">
                <li v-for="task in tasks" v-if="task.is_done" v-bind:id="'row_task_' + task.id" v-cloak class="collection-item">
                    <input type="checkbox" v-bind:id="'task_' + task.id" checked="checked" />
                    <label v-bind:id="'task_' + task.id" class="line-through">{{ task.name }}</label>
                </li>
            </ul>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        moveFinishedTask: function (task_id) {
            ...
            var li = document.querySelector('#finished-tasks ul li:finished-child');
